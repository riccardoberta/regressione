...
Quattro notebook progressivi. Ogni notebook è autonomo: scarica il dataset, applica la pulizia e introduce i concetti necessari. La narrazione è continua, ogni notebook chiude con un cliffhanger ripreso dal successivo.

| # | Notebook | Tema | Colab |
|---|---|---|---|
| 1 | `01_il_mistero_del_valore.ipynb` | Esplorazione dei dati, intuizioni visive, coefficiente di correlazione di Pearson, distinzione fra correlazione, importanza e causalità | [![Open In Colab](https://colab.research.google.com/assets/colab-badge.svg)](https://colab.research.google.com/github/riccardoberta/regressione/blob/main/01_il_mistero_del_valore.ipynb) |
| 2 | `02_primo_scout_automatico.ipynb` | Regressione lineare semplice, funzione costo visualizzata in 3D, discesa del gradiente, residui, MAE e confronto con baseline casuale | [![Open In Colab](https://colab.research.google.com/assets/colab-badge.svg)](https://colab.research.google.com/github/riccardoberta/regressione/blob/main/02_primo_scout_automatico.ipynb) |
| 3 | `03_lo_scout_migliora.ipynb` | Regressione lineare multipla, uso di scikit-learn (SGDRegressor, MinMaxScaler, Pipeline), normalizzazione delle feature, importanza relativa dei coefficienti, regressione polinomiale, primo sguardo agli alberi di decisione come modelli non parametrici | [![Open In Colab](https://colab.research.google.com/assets/colab-badge.svg)](https://colab.research.google.com/github/riccardoberta/regressione/blob/main/03_lo_scout_migliora.ipynb) |
| 4 | `04_generalizzazione_overfitting.ipynb` | Errore empirico vs errore atteso, separazione train/test, alberi di decisione "potati", overfitting, equilibrio bias–varianza | [![Open In Colab](https://colab.research.google.com/assets/colab-badge.svg)](https://colab.research.google.com/github/riccardoberta/regressione/blob/main/04_generalizzazione_overfitting.ipynb) |
...
- confrontare modelli diversi usando MAE;
...
- **Cosa dovremmo aver capito** — sintesi dei concetti chiave del notebook appena concluso;
...
- modello lineare e sua estensione multivariata;
...
2. Prima dell'inizio della lezione, esegui *Runtime → Esegui tutto* (`Ctrl+F9` / `Cmd+F9`) per scaricare il dataset e popolare le celle: alla prima esecuzione l'archivio FIFA viene scaricato da Dropbox, poi tutto è in cache.
...
I notebook usano il **FIFA 22 dataset** ([Kaggle](https://www.kaggle.com/datasets/luisfucros/fifa-players), licenza MIT). Per garantire la riproducibilità in aula, ogni notebook scarica automaticamente una copia congelata da Dropbox alla prima esecuzione: non serve registrarsi a Kaggle né caricare file manualmente.
...
Il dataset originale è di [EA Sports](https://www.ea.com/games/fifa) ed è pubblicato su [Kaggle](https://www.kaggle.com/datasets/luisfucros/fifa-players) con licenza MIT.
